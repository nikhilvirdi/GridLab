...
exports.solveBidirectionalBFS = solveBidirectionalBFS;
 ...
 */
function solveBidirectionalBFS(req) {
    ...
    const parentFwd = new Map([[srcKey, null]]);
    const parentBwd = new Map([[dstKey, null]]);
    ...
    const visitedOrder = [src, dst];
    const visitedSet = new Set([srcKey, dstKey]);
    let meetingKey = null;
    /**
     * Expand one level of the given frontier.
     * Returns the meeting node key if the frontiers have intersected, or null.
     */
    function expandLevel(queue, head, ownParent, otherParent) {
    ...
    const bwdPath = [];
    let bcur = parentBwd.get(meetingKey) ?? null;
    while (bcur !== null) {
